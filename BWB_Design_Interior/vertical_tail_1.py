...
class VerticalTail(GeomBase):
    ...
    nb_pts_twist = Input(2)
    angles_twist = Input([10, 10])  # degrees of twist
    y_twist_except_last = Input([0])  # location of the definition of twist
    nb_pts_twist_axis = Input(2)
    x_twist_axis = Input([0, -1])
    y_twist_axis_except_last = Input([0, 3])
    ...
    def initial_surface(self):
        ...
        return Surface_BWB(nb_eq_pts_rails = self.nb_eq_pts_rails,
                          ...
                          nb_pts_twist = self.nb_pts_twist,
                          angles_twist = self.angles_twist,
                          y_twist_except_last = self.y_twist_except_last,
                          nb_pts_twist_axis = self.nb_pts_twist_axis,
                          x_twist_axis = self.x_twist_axis,
                          y_twist_axis_except_last = self.y_twist_except_last,
    ...
    @Attribute
    def y_fcp(self):
        return self.initial_surface.y_fcp
    ...
    def position_surf1_on_surf2(self):
        ...
        lst_inter_te = LineSegment(start = crvs_surface_vt_lst[0].sample_points[int(self.nb_eq_pts_airfoils_vt*0.7)],
                                       end = Point(crvs_surface_vt_lst[0].sample_points[int(self.nb_eq_pts_airfoils_vt*0.7)][0],
                                                   crvs_surface_vt_lst[0].sample_points[int(self.nb_eq_pts_airfoils_vt*0.7)][1],
                                                   -1e3)).surface_intersections(
                surf2)
        pt_attachment_te_vt_projected = lst_inter_te[0]['point']
        dz_old = 1e3
        ...
        return crvs_surface_vt_lst, AoA_vb

    @Attribute
    def all_sections(self):
        """
        Returns the AVL sections that corresponds (for the vertical base) to the curves defining the vertical tail
        positioned on the surface 'surf2'.
