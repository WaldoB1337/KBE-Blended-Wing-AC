...
class VerticalBase(GeomBase):
    ...
    def initial_surface(self):
        ...
        return Surface_BWB(nb_eq_pts_rails = self.nb_eq_pts_rails,
                          ...
                          sweeps_te = [10, 10],#no need
                          sspan = self.span,
                          x_fcp_except_last = self.x_fcp_except_last,
                          y_fcp_except_last = self.y_fcp_except_last,
                          chords = self.chords,
                          rho_le = [.01, .01],#no need
                          rho_te = [.01, .01],#no need
                          nb_pts_twist = 2,#no need
                          angles_twist = [0, 0],#no need
                          y_twist_except_last = [0, 0],#no need
                          nb_pts_twist_axis = 2,#no need
                          x_twist_axis = [0, -0.876701437037684],#no need
                          y_twist_axis_except_last = [0, 0.4],#no need
                          nb_pts_dihedral = self.nb_pts_dihedral,
                          angles_dihedral = self.angles_dihedral,
                          spans_dihedral_except_last = self.spans_dihedral_except_last,#no need
                          weights_dihedral = [.1, .1, .1, .1],#no need
                          ...
                          cant_angles_airfoils = self.cant_angles_airfoils,#no need
    ...
    @Attribute
    def crvs_vb_surface_lst(self):
        return self.initial_surface.crvs_surface_all_airfoils

    @Attribute
    def translate_crvs_surface(self):
        crvs_vb_surface_lst = self.crvs_vb_surface_lst[:]
        for i in range(self.nb_airfoils):
            crvs_vb_surface_lst[i] = TranslatedCurve(crvs_vb_surface_lst[i], Vector(self.x_vb, self.y_vb, 10))
        return crvs_vb_surface_lst
    ...
    def position_surf1_on_surf2(self):
        pt_attachment_vb = self.define_attachment_pt_object
        crvs_surface_vb_lst = self.translate_crvs_surface[:]
        ...
        lst_inter_le = LineSegment(start = pt_attachment_vb,
                                   end = Point(pt_attachment_vb[0], pt_attachment_vb[1], -1e3)).surface_intersections(
            surf2)
        pt_attachment_le_vt_projected = lst_inter_le[0]['point']
        vect_displ_vt = Vector(pt_attachment_le_vt_projected[0] - pt_attachment_vb[0],
                               pt_attachment_le_vt_projected[1] - pt_attachment_vb[1],
                               pt_attachment_le_vt_projected[2] - pt_attachment_vb[2])
        ...
        for i in range(len(crvs_surface_vb_lst)):
            crvs_surface_vb_lst[i] = TranslatedCurve(crvs_surface_vb_lst[i], vect_displ_vt)
        ...
        dz_old = 1e3
        dz_new = crvs_surface_vb_lst[0].start[2] - pt_attachment_te_vt_projected[2]
        ...
        AoA_vb = self.dtheta1 * count

            # Placement 3rd point non colinear on the based airfoil
        count = 0
        crvs_vt_rotation_2_lst = []

        lst_inter_te = LineSegment(start = crvs_surface_vb_lst[0].sample_points[int(115)],
                                       end = Point(crvs_surface_vb_lst[0].sample_points[int(115)][0],
                                                   crvs_surface_vb_lst[0].sample_points[int(115)][1],
                                                   -1e3)).surface_intersections(
                surf2)
        pt_attachment_te_vt_projected = lst_inter_te[0]['point']
        dz_old = 1e3
    ...
    def crvs_surface_to_points(self):
        ...
        pts_surface_lst = []
        for i in range(self.nb_airfoils):
            pts_airfoil_lst = crvs_surface_lst[i].equispaced_points(self.nb_eq_pts_airfoils_vb)
